...
const department: DepartmentView = {
  ...
  root_id: 'HOD',
  employees: [
    { employee_id: 'HOD', name: 'Head', role: 'HOD', monthly_salary: 1, manager_id: null, children_ids: ['MGR_C', 'MGR_A', 'MGR_B'], direct_report_count: 3, team_headcount: 8, team_payroll: 8 },
    ...
    { employee_id: 'LEAD_A', name: 'Lead', role: 'Lead', monthly_salary: 1, manager_id: 'MGR_A', children_ids: ['E2', 'E1'], direct_report_count: 2, team_headcount: 3, team_payroll: 3 },
...
describe('layoutTree', () => {
  it('orders siblings by children_ids even when employee records are scrambled', () => {
    ...
    expect(['MGR_C', 'MGR_A', 'MGR_B'].map(xOf)).toEqual([0, 324, 648])
    expect(['E2', 'E1'].map(xOf)).toEqual([216, 432])
  })
  ...
  it('centres each parent between its first and last child', () => {
    ...
    expect(xOf('HOD')).toBe((xOf('MGR_C') + xOf('MGR_B')) / 2)
    expect(xOf('LEAD_A')).toBe((xOf('E2') + xOf('E1')) / 2)
  })
